...
module dtc_gain_lms_cal #(
    ...
    parameter ACCUM_WIDTH   = 24       // LMS 累加器位宽（决定收敛速度）
) (
    input  wire                       clk,           // 更新时钟（sys_clk_g）
    input  wire                       rst_n,         // 异步复位
    input  wire                       sign_in,       // 相位符号（0=ref超前=过补偿,
                                                     //             1=fb超前=欠补偿）
    input  wire  [CODE_WIDTH-1:0]     dtc_code_in,   // DTC 码字（无符号，用于提取码流方向）
    ...
    wire signed [GAIN_ADJ_WIDTH-1:0] gain_adjust = accum_next[ACCUM_WIDTH-1 -: GAIN_ADJ_WIDTH];

    // 有符号增益求和（在 always 外算一次，全程用 signed 比较/截取）
    // 注意: gain_base 本身是无符号, 必须显式 $signed 转成有符号再相加,
    // 否则 "无符号 gain_base + 有符号 gain_adjust" 会按无符号算, 减法出错。
    wire signed [GAIN_BASE_WIDTH:0] gain_sum = $signed({1'b0, gain_base}) + gain_adjust;
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // 饱和输出: 全程用有符号 gain_sum 比较/截取
            // (原版阈值 {1'b0,{GAIN_BASE_WIDTH{1'b1}}} 是无符号, 与有符号 gain_sum 比较时
            //  511>=1023 会被误判为真, 把 gain 直接饱和到 1023 —— 表现为"减到 512 再减就跳 1023")
            if (gain_sum >= $signed({1'b0, {GAIN_BASE_WIDTH{1'b1}}}))
