...
echo ==========================================
echo  3rd-Order MASH Quick Simulation
echo  Short run, selective dump, small VCD
echo ==========================================

set SRC=src
set MOD=models

iverilog -g2012 -o main_quick_3rd.vvp ^
    %MOD%\carry8_model.v ^
    xilinx_sim.v ^
    vio_override.v ^
    %SRC%\dtc\lut_dtc.v ^
...
vvp main_quick_3rd.vvp
if errorlevel 1 (
    echo [FAIL] Simulation error
    pause
    exit /b 1
)
echo [OK] Simulation complete - VCD: tb_quick_3rd.vcd
echo Opening GTKWave...
start "" gtkwave tb_quick_3rd.vcd
pause
